...
const audit=[{id:"A-001",severity:"BLOCKER",finding:project.blocker,status:"OPEN"}];
...
function authorized(req){if(!ADMIN_TOKEN)return false; const h=req.headers.authorization||""; return h===`Bearer ${ADMIN_TOKEN}`;}
...
const server=http.createServer(async(req,res)=>{
 ...
 if(req.method==="GET"&&u.pathname==="/api/v1/project")return json(res,200,project);
 if(req.method==="GET"&&u.pathname==="/api/v1/gates")return json(res,200,gates);
 if(req.method==="GET"&&u.pathname==="/api/v1/teams")return json(res,200,teams);
 if(req.method==="GET"&&u.pathname==="/api/v1/tasks")return json(res,200,tasks);
 if(req.method==="GET"&&u.pathname==="/api/v1/decisions")return json(res,200,decisions);
 if(req.method==="GET"&&u.pathname==="/api/v1/audit")return json(res,200,audit);
 if(req.method==="POST"&&u.pathname==="/api/v1/gates/advance"){if(!authorized(req))return json(res,403,{error:"admin_authorization_required"});return json(res,409,{error:"gate_blocked",gate:project.currentGate,reason:project.blocker})}
 json(res,404,{error:"not_found"});
});
initDb().catch(e=>console.error("db_init_failed",e.message));
